...
function spawnNextPiece() {
    ...
    if (collide()) {
        // If a new piece collides immediately -> Game Over
        ...
        piece.forEach((row, y) => {
            row.forEach((value, x) => {
                if (value) {
                    ...
                    ctx.fillRect(offsetX + x * previewBlockSize + 2, offsetY + y * previewBlockSize + 2, 3, 3);
                    ctx.fillStyle = color;
                }
            });
        });
...
function rotate() {
    if (!currentPiece || isResolvingLock) return;
    playSound("rotate");
...
function clearLines(completedRows = getCompletedRows()) {
    const rowsToClear = new Set(completedRows);
    const lines = rowsToClear.size;

    if (lines > 0) {
        const remainingRows = board.filter((_, rowIndex) => !rowsToClear.has(rowIndex));
        const emptyRows = Array.from({ length: lines }, () => Array(cols).fill(0));
    ...
    for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
            if (board[y][x]) {
                ...
                boardCtx.fillRect(x * blockSize, y * blockSize, blockSize - 2, blockSize - 2);
            }
        }
    }
}
...
function update(time = 0) {
    if (!isPaused) {
        ...
        lastTime = time;
        if (!isResolvingLock && currentPiece) {
...
function drop() {
    if (!currentPiece || isResolvingLock) {
        dropCounter = 0;
        return;
    }
...
document.addEventListener("keydown", (event) => {
    if (!currentPiece && event.key.toLowerCase() !== "p") return;
    ...
    switch (event.key) {
        ...
        case "ArrowUp":
            ...
            break;
        case " ":
            if (event.repeat || !currentPiece || isResolvingLock) break;
            while (!collide()) {
                currentPiecePos.y++;
            }
            currentPiecePos.y--;
...
function bootSequence() {
    ...
    const bootLines = ["SYSTEM BOOT v3.7", "INITIALIZING...", "LOADING TETRIS OS", "READY."];
    ...
    function playLineCompleteSound(lineIndex) {
        ...
        completeOsc.start();
        completeGain.gain.exponentialRampToValueAtTime(0.001, audioContext.currentTime + 0.3);
